...
class RFM69:
    ...
    def sample_rssi(self, timeout_ms=50):
        """ Make a request to radio module for sampling the rssi.
        ...
        # 1) Wait for RxReady (bit6 in RegIrqFlags1)
        start = ticks_ms()
        while (self.spi_read(RFM69_REG_IRQ_FLAGS1) & 0x40) == 0:
            if ticks_diff(ticks_ms(), start) >= timeout_ms:
                return None  # or raise TimeoutError("RxReady timeout")

        # 2) Force a 0→1 edge on RSSI_START
        self.spi_write(RFM69_REG_RSSI_CONFIG, 0x00)
        self.spi_write(RFM69_REG_RSSI_CONFIG, RF_RSSI_START)

        # 3) Wait for RSSI_DONE (bit1), with timeout
        start = ticks_ms()
        while (self.spi_read(RFM69_REG_RSSI_CONFIG) & RF_RSSI_DONE) == 0:
            if ticks_diff(ticks_ms(), start) >= timeout_ms:
                return None  # or raise TimeoutError("RSSI_DONE timeout")
